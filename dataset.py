...
def move_points(vertices, index1, index2, r, coef):
    ...
    # index correspond to 2 elements(x, y) in vertices
...
def shrink_poly(vertices, coef=.3):
...
def is_cross_text(start_loc, length, vertices):
    ...
    for vertice in vertices:
        ...
        if p2.area == 0:
            p2_area = 1e-7  # 0.0000001
            # print("p2.area=%s,inter = %s"%(p2_area,inter))

        if .01 <= inter / p2_area <= .99:
            return True
...
def rotate_img(img, vertices, angle_range=10):
    ...
    # PIL.Image.Image.rotate(), rotate in clockwise direction
...
class custom_dataset(data.Dataset):
    def __init__(self, img_path, gt_path, scale=.25, length=512):
